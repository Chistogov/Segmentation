...
# local imports
from ants.sampling import DataLoader, CSVDataset
from ants.sampling import transforms as tx
from ants.models import create_unet_model2D


data_dir = 'reflex/train/'
results_dir = 'results_2D/test/'

# tx.Compose lets you string together multiple transforms
co_tx = tx.Compose([tx.MinMaxScaler((-1,1)),
                    tx.ExpandDims(axis=-1), # expand from (128,128) to (128,128,1) -> RandomAffine and Keras expect that
...
callbacks = [cbks.ModelCheckpoint(results_dir+'ae-weights-py.h5', monitor='val_loss', save_best_only=True),
